...
import { getDashboardCounts } from "../../actions/AdminAuthAction";
...
class Dashboard extends Component {
  ...
  getDashboardCounts = async () => {
    await this.props.getDashboardCounts(reqData);
    // console.log("propppKunj---", this.props.DashboardCountsData);
    if (this.props.DashboardCountsData && this.props.DashboardCountsData.data) {
      await this.setState({
        userDashData: {
          ...this.state.userDashData,
          total: this.props.DashboardCountsData.data.dashboardData.user,
        },
        programData: {
          ...this.state.programData,
          total: this.props.DashboardCountsData.data.dashboardData.program,
        },
      });
    }
  };
  ...
  getUsersDetails = async () => {
    let reqData = {
      page_no: this.state.activePage, //i.e 1
      limit: this.state.limit, //i.e 10
    };

    this.state.load_status !== ""
      ? (reqData.load_status = this.state.load_status)
      : null;
    ...
    if (this.state.status == "") {
      ...
      if (this.state.status == "In transit") {
        if (this.state.eta_from == "") {
          ...
          reqData["eta_from"] = moment(this.state.eta_from).format(
            "MM/DD/YYYY"
          );
  ...
  toggleModal = (id, status, eta_from, eta_to) => {
    if (
      status != "Pending Truck Assignment" &&
      status != "In storage" &&
      status != "Picked up" &&
      status != "Shipped" &&
      status != "In transit" &&
            ...
            <div className="row">
              <div className="col-xs-12 col-sm-6 col-md-6 col-lg-4 col-xl-3">
                <Link
                  to={{
                    pathname: "/dashboard",
                    // state: { params: { load_status: '3' } }
                  }}
                >
                  <LineChart data={this.state.userDashData} />
                </Link>
              </div>
              <div className="col-xs-12 col-sm-6 col-md-6 col-lg-4 col-xl-3">
                <Link
                  to={{
                    ...
                    // state: { params: { load_status: '0' } }
                  }}
                >
                  <LineChart data={this.state.programData} />
                </Link>
              </div>
            </div>
          </div>
                      ...
                      <tr key={key}>
                        ...
                        <td className="d-flex justify-content-center">
                          <Switch
                            ...
                            onChange={(e) => this.toggleStatus(e, user.user_id)}
                          />
                        </td>
                  ...
                  <p>
                    ...
                    {this.state.totalUserCount > 0
                      ? this.state.activePage * AppConfig.PER_PAGE -
                        ...
                        1
                      : 0}{" "}
                    {/* i.e : activePage : 1 * PER_PAGE : 10 + 1 = 11 to{" "} */}
                    to&nbsp;
                    {/* i.e PER_Page : 10 * activePage : 1 - (10-lenght of data (10) = Ans is 10) means show the 10 data  */}
                    {AppConfig.PER_PAGE * this.state.activePage -
                      (AppConfig.PER_PAGE - this.state.userData.length)}{" "}
                    of {this.state.totalUserCount} entries
                  </p>
                </Col>
                <Col sm="6" md="6">
...
// map state to props
const mapStateToProps = ({ adminAuthReducer, userReducer }) => {
  const { DashboardCountsData } = adminAuthReducer;
  const { getUsersDetailsData, activeDeactiveUserData } = userReducer;
  return { DashboardCountsData, getUsersDetailsData, activeDeactiveUserData };
};
